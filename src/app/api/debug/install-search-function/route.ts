...
/**
 * Endpoint para instalar a função search_vehicles no banco de produção
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // SQL da função search_vehicles (versão simplificada)
...
$$;
    ...
    console.log('Instalando função search_vehicles...');

    // Executar a função SQL usando uma query direta
    const { error } = await supabase
      .from('vehicles')
      .select('id')
      .limit(1); // Query simples para testar conexão

    if (error) {
      return NextResponse.json(
        { error: 'Erro de conexão com o banco', details: error },
        { status: 500 }
      );
    }

    // Para instalar a função, você precisará executar o SQL diretamente no Supabase
    // Este endpoint serve para verificar se a função existe e testar

    // Testar se a função foi criada
    const { data: testResult, error: testError } = await supabase.rpc('search_vehicles', {
      p_search_text: null,
      p_states: null,
      p_cities: null,
      p_brands: null,
    ...
    return NextResponse.json({
      success: !testError,
      message: testError ? 'Função search_vehicles não existe no banco' : 'Função search_vehicles encontrada',
      error: testError,
      testResult: testResult?.length || 0,
      sqlFunction: searchFunctionSQL,
